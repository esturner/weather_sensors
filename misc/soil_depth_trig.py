...
Triggers i2c for each measurement
'''
import numpy as np
...
#set up trigger ---------
pini2c = 8 #pin to output 3.3V (HIGH)
GPIO.setmode(GPIO.BCM) #setup GPIO
GPIO.setup(pini2c, GPIO.OUT) #setup pin for output

#i2c----------------------
GPIO.output(pini2c, 1) #turn on i2c for setup
sleep(3)
...
GPIO.output(pini2c, 0) #turn off setup
#Data Storage-----------------------
...
start = datetime.now()
interval = 60 #minutes
duration = 60*24*5 #minutes
i=0
while(i*interval)<duration:
	GPIO.output(pini2c, 1) #turn on i2c
	sleep(5)
	...
	for sensor in soil_sensors:
		...
		sensor.ledOff()
		time.sleep(1)
	GPIO.output(pini2c, 0) #turn off i2c pin
	#data log
